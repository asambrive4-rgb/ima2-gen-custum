...
export function AgentImageSheet({ open, currentImage, images, activeTab, onTabChange, onClose }: Props) {
  const { t } = useI18n();
  ...
  return (
    <div className="agent-dialog agent-dialog--image" role="presentation">
      ...
      <section ref={panelRef} className="agent-image-sheet" role="dialog" aria-modal="true" aria-label={t("agent.imagePane")}>
        ...
        <AgentImagePane currentImage={currentImage} images={images} activeTab={activeTab} onTabChange={onTabChange} />
      </section>
    </div>
  );
}
